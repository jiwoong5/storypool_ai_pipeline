...
/ocr/process 
- 입력: file: UploadFile = File(...), reader_type: str
- 출력: OCRResponse 모델(status, message, 처리 시간, 출력 디렉토리, 추출된 텍스트, confidence_score, detected_languages)
- 수행내용: 출력 디렉토리 생성, 입력 파일 저장, ocr 처리, 결과 파일 읽기, 응답 생성
- ocr 처리: image_path, output_path 입력 => ocr 결과(.txt)를 output path에 저장
