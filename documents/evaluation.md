...
- 항목5: 유진 <-> 엄마 (실제: 엄마 <-> 유진 <-> 가장 큰 초코 팩) > 유진 <-> 봉지 (엄마 <-> 유진 <-> 거울, 봉지) => 1점
=> 총점 6/16
...
개선 방식
- scene_parser, prompt_maker prompt 개선: 기존 완결된 one-shot 모델 사용에서 출력 형식 예시만 제공하는 형태로 개선으로 인한 입력예시편향 제거 > 각 항목 점수 개선
- prompt_maker location 로직 분리: location inference 로직 분리로 scene parsing, location inference 집중도 향상 > location 관련 점수 및 관계성 점수 향상
- prompt_maker prompt 개선: 기존 인물들 관계에 집중했던 프롬프트에서 오브젝트 추가 고려 > 오브젝트 포함 점수 향상
- prompt_maker 로직 분리: 그림체, character 묘사, costume 묘사 로직을 분리하고 해당 내용을 후처리 과정에 포함시키면서 각 항목 집중도와 일관성 향상 > 등장인물 묘사, 그림체 일관성 점수 향상
- scene_parser 후처리 로직 추가: 누락된 문장에 대해서 후처리로 앞, 또는 뒤의 scene 에 추가하는 로직 > prompt 생성 정밀도가 높아지면서 각 항목 점수 향상
